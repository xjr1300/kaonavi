...
`ルート・ディレクトリ`にカレント・ディレクトリを移動して、に以下の通り仮想環境を作成します。
...
上記の通り仮想環境を作成したとき、仮想環境が有効になると、ターミナルのプロンプトの先頭が`(venv)`になります。

> これ以降の說明は、仮想環境を有効にしたルート・ディレクトリで作業します。
...
`KAONAVI_API_ENDPOINT`は、他の環境変数から導出するため、設定する必要はありません。

カオナビのコンシューマー・キーとコンシューマー・シークレットは、カオナビの`カオナビ管理者機能トップ - 公開API v2 情報 - 認証情報`で確認できます。
